...
parser = argparse.ArgumentParser(
    description='Convert a distance tree into time tree with distances in days.'
)
parser.add_argument(
    '-t',
    help=
...
parser.add_argument(
    '-m',
    help='A metadata file with columns strain and date (in 2020-01-02 format)',
    required=True)
...
parser.add_argument('-lr', default=0.1, type=float, help="Adam learning rate")
...
def calc_dates(branch_lengths_array):
    ...
    calc_dates = helpers.sp_matmul(A, B, final_terminal_dimension).squeeze()
    return calc_dates
